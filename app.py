...
def get_address_from_coords(lat: float, lon: float) -> str:
    """Reverse geocode lat/lon into a street address."""
    try:
...
def parse_pdf_report(file_bytes: bytes) -> dict:
    """Best-effort parser for a PDF generated by this app."""
    try:
...
# --- CSS (simplified, readable dropdowns) ---
st.markdown(
    f"""
<style>
...
if "site_address_input" not in st.session_state:
    st.session_state["site_address_input"] = ""
            ...
            if not loc:
                ...
                if err:
                    st.warning(
                        ...
                        "Use the map click instead, or allow location in your browser."
                    )
                else:
...
# Optional: upload existing PDF report to pre-fill
with st.expander("Upload completed PDF report to pre-fill form (optional)", expanded=False):
    ...
    if pdf_file is not None:
        ...
        if not parsed:
            ...
            st.error(
                "Failed to parse the uploaded PDF. It may be corrupted or scanned as images."
            )
        else:
...
with st.form("site_form", clear_on_submit=False):
    ...
    with c_gps1:
        gps_lat = st.text_input(
            "GPS latitude", value=st.session_state["gps_lat"], key="gps_lat_input"
        )
    with c_gps2:
        gps_lon = st.text_input(
            "GPS longitude", value=st.session_state["gps_lon"], key="gps_lon_input"
        )
    with c_gps3:
    ...
    current_gps_lat = gps_lat.strip() if gps_lat else ""
    current_gps_lon = gps_lon.strip() if gps_lon else ""
    current_coords = (
        (current_gps_lat, current_gps_lon)
        if current_gps_lat and current_gps_lon
        else None
    )
    ...
    if current_coords and current_coords != st.session_state.get("last_geocoded_coords"):
        try:
            ...
            st.session_state["auto_address"] = addr or ""
            st.session_state["last_geocoded_coords"] = current_coords
            if addr:
                st.session_state["site_address_input"] = addr
        except Exception:
            st.session_state["last_geocoded_coords"] = None

    if not st.session_state["site_address_input"]:
        st.session_state["site_address_input"] = (
    ...
    with cc3:
        ...
        depth_check_within_tol = abs(depth_check_diff_mm) <= depth_check_tolerance_mm
        st.markdown(
    ...
    if submitted:
        ...
        # Enforce uniqueness on site_name
