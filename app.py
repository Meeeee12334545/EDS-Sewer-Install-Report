...
with st.form("site_form", clear_on_submit=False):
    ...
    # Auto-populate address from map click if lat/lon are set in session state and have changed
    current_coords = (st.session_state["gps_lat"], st.session_state["gps_lon"]) if st.session_state["gps_lat"] and st.session_state["gps_lon"] else None
    if current_coords and current_coords != st.session_state.get("last_geocoded_coords"):
        try:
            addr = get_address_from_coords(float(st.session_state["gps_lat"]), float(st.session_state["gps_lon"]))
            if addr:
    ...
    # Site address field (auto-populated from map, but editable)
